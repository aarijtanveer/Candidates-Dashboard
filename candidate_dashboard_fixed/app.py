...
def load_data(uploaded_file):
    ...
    if uploaded_file is not None:
        ...
        sample_path = DATA_DIR / 'Dashboard Format.csv'
        if sample_path.exists():
            df = pd.read_csv(sample_path, **opts, on_bad_lines='skip')
        else:
            st.warning(f"Couldn't find sample CSV at {sample_path}. Please upload a file from the sidebar.")
            return pd.DataFrame()
...
with st.container():
    ...
    with c4:
        ...
        st.write("**Experience Cities**", cities)
