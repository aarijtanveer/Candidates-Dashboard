...
def apply_theme():
    ...
    :root{{
      ...
      --text:{text}; --muted:{muted}; --bg:{bg}; --card:{card}; --border:{border};
      --font:{t['font']}, -apple-system, Segoe UI, Roboto, "Helvetica Neue", Arial, sans-serif;
      --base-scale:{t['base_scale']};
      --radius:12px;
      --shadow: 0 8px 24px rgba(0,0,0,.08);
    }}
    html, body, [class^="css"] {{ font-family: var(--font); color: var(--text); font-size: calc(16px * var(--base-scale)); }}

    /* Layout */
    section.main > div {{ background: var(--bg); }}
    .card {{ background: var(--card); border: 1px solid var(--border); border-radius: var(--radius); box-shadow: var(--shadow); padding: 14px 16px; margin-bottom: 14px; }}
    .header-band {{ background: linear-gradient(90deg, var(--primary) 0%, var(--accent) 100%); color:#fff; border-radius: var(--radius); padding: 16px 18px; box-shadow: var(--shadow); margin-bottom: 12px; }}
    .header-band .title {{ font-weight: 800; font-size: 1.8rem; line-height:1.1; }}
    .header-band .subtitle {{ opacity:.95; font-weight:600; margin-top:.2rem; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; max-width:85ch; }}
    .pills {{ display:flex; gap:.4rem; flex-wrap:wrap; margin-top:.5rem; }}
    .pill {{ background:#ffffff22; color:#fff; padding:.3rem .6rem; border:1px solid #ffffff55; border-radius: 999px; font-size:.85rem; }}

    .kpi {{ text-align:center; border-radius: 10px; border:1px solid var(--border); padding:.9rem; background: var(--card); }}
    .kpi .value {{ font-size: 1.7rem; font-weight: 800; color: var(--primary); }}
    .kpi .label {{ font-size:.85rem; color: var(--muted); font-weight:600; }}

    .section-title {{ font-weight: 800; color: var(--text); border-left:4px solid var(--primary); padding-left:8px; margin-bottom:.6rem; }}

    .qa {{ display:grid; grid-template-columns: 220px 12px 1fr; align-items:center; gap:.4rem; padding:.25rem .3rem; }}
    .qa .qa-label {{ color: var(--muted); font-weight:700; }}
    .qa .qa-sep {{ height:1px; background: linear-gradient(90deg, transparent, var(--border), transparent); }}
    .qa .qa-value {{ color: var(--text); font-weight:700; }}

    .role {{ border:1px dashed var(--border); border-radius: 10px; padding:.6rem .7rem; }}
    .role-top {{ display:flex; justify-content:space-between; align-items:flex-start; gap:.6rem; }}
    .role-left {{ max-width:62%; }}
    .role-title {{ font-weight:800; color: var(--text); }}
    .role-org {{ font-weight:800; color: var(--text); text-align:right; max-width:38%; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }}
    .role-meta {{ margin-top:.35rem; display:flex; gap:.4rem; flex-wrap:wrap; }}
    .chip {{ background: #F1F5F9; border:1px solid var(--border); color:#0F172A; padding:.2rem .5rem; border-radius:999px; font-size:.8rem; }}

    .stat { text-align:center; border:1px solid var(--border); border-radius:10px; padding:.6rem .8rem; }
    .stat-label { color: var(--muted); font-weight:700; font-size:.85rem; }
    .stat-value { color: var(--primary); font-size:1.6rem; font-weight:900; }
    </style>
    """
